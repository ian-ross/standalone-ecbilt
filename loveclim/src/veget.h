...
!
      ...
      common /veg_netcdf/newvegvar,veg_fill_value,veg_missing_value, &
           & numvegvar,namevegvar,phi

!---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
! 2 files "bio.inc" & "buffer.inc" combined in one : "veget.h" (09/12/99)
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
...
!jmc_D
!       COMMON /CLIMATE/ TATB(nlat,nlon),  PRCB(nlat,nlon),
!    >   PRCB5(nlat,nlon), GDD0B(nlat,nlon)
!jmc_F
        COMMON /BIOTA/ &
     ...
     &   SNLTR(nlat,nlon)
        ...
        COMMON /BIOTA2/ ANUPLOCH(nlat,nlon), STOCKLOCH(nlat,nlon)

       common /vegetint/ ieqveg,ieqvegwr,iscendef,ivegstrt
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
!-AM (2007)
! CO2 enrichment :
!  betat for tree, betag for grass
!  nppt = tree npp, nppg = grass npp
       REAL*8 BETAG,BETAT,NPPG,NPPT
       COMMON /RICHE/BETAG,BETAT,NPPG,NPPT
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
!-AM (2008)
! Deforestation scenario (data read in VEGET.dat):
!  farea = fraction of the mesh occupied by crops according to R&F (1999)
!  ndfor = nbr of records in deforestation scenario (ndfor =< mdfor)
!  i0dfor = year-1 of first available data in VEGET.dat (date A.D.)
! Reference vegetation distribution against which deforestation is evaluated:
!  sd_const(nlat,nlon) = reference desert distribution.
!  st_const(nlat,nlon) = reference tree distribution.
! Would CO2 flux from vegetation influence atm. CO2 or not : fco2veg
