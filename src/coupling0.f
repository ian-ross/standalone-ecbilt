...
c-----------------------------------------------------------------------
      ...
      integer ist,jst,i,j
#if ISOTOPE == 1
      integer isokof,cmymms
#endif
      real*8  ratm,facstr,rndws,dwsm1,facsnow
#if ISOTOPE == 1
      parameter (isokof=6e-3)
      cmymms= 86400.*360./10.
#endif
...
c      do j=1,nlon
c	do i=1,nlat
c	  if (fracto(i,j).gt.epss) then
c - ISOTOPES starts ..
*
*** Calculate the isotopic content in precipitation
*
c            isotem(i,j) = tempsg(i,j)-tzero
c            isotem(i,j)=tempsg(i,j)
c            doxpre(i,j) = -11.88+0.345*tempsg(i,j)-(couprf(i,j)+coupsf(i,j))*
c     &                    0.0022
c            ddepre(i,j) = doxpre(i,j)/8.
c            write(99,*)'tempsg146',tempsg(14,6)
c            write(99,*)'tzero',tzero
c            write(99,*)'couprf146',couprf(14,6)
c           write(99,*)'coupsf146',coupsf(14,6)
c           write(99,*)'doxpre',doxpre
*
*** Calculate the fractionation factor, isotff=ln(alpha)
*
c            isotff(i,j) = (1.137e3/tempsg(i,j)**2)-
c    &                     (0.4156/tempsg(i,j))-2.0667e-3
c            isotff(i,j) = exp(isotff(i,j))
c           write(99,*)'isotff(8,8)',isotff(8,8)
*
*** Calculate the isotopic content in vapor
*
c            doxvap(i,j) = (1./isotff(i,j))*doxpre(i,j)+(1./isotff(i,j)-1.)
c            ddevap(i,j) = doxvap(i,j)/8.
c           write(99,*)'doxvap88',doxvap(8,8)
c           write(99,*)'ddevap88',ddevap(8,8)
*
*** The isotopic content in surface ocean waters
*
c            doxssw(i,j) = 0.
c            ddessw(i,j) = 0.

*** Calculate the isotopic content in evaporation
*
c            doxeva(i,j) =( (1.-isokof)/(1.-relhum(i,j))*(1./isotff(i,j)*
c     &                    (doxssw(i,j)+1.)-(doxvap(i,j)+1.)*
c     &                    relhum(i,j)) )-1.
c            ddeeva(i,j) =( (1.-isokof)/(1.-relhum(i,j))*(1./isotff(i,j)*
c     &                    (ddessw(i,j)+1.)-(ddevap(i,j)+1.)*
c     &                    relhum(i,j)) )-1.
c      write(99,*)'doxeva(8,8)',doxeva(8,8)
c      write(99,*)'ddeeva88',ddeeva(8,8)
*
*** The isotopic content in runoff
*
c            doxrof(i,j) = doxpre(i,j)
c            dderof(i,j) = ddepre(i,j)
*
*** For cheking if the equations are correct
*
c            doxeva(i,j)=5.
c            doxpre(i,j)=5.
c            ddeeva(i,j)=7.
...
#if ISOTOPE == 1
            sumoiof(i,j) = sumoiof(i,j) +
     &                     ( ((evapn(i,j,noc)*fractn(i,j,noc)+
     &                     evapn(i,j,nse)*fractn(i,j,nse))/fracto(i,j))*
     &                     doxeva(i,j)-(couprf(i,j)+coupsf(i,j))*
     &                     doxpre(i,j)-sumro(i,j)*doxrof(i,j))
            sumoidf(i,j) = sumoidf(i,j) +
     &                     ( ((evapn(i,j,noc)*fractn(i,j,noc)+
     &                     evapn(i,j,nse)*fractn(i,j,nse))/fracto(i,j))*
     &                     ddeeva(i,j)-(couprf(i,j)+coupsf(i,j))*
     &                     ddepre(i,j)-sumro(i,j)*dderof(i,j))
#endif
...
#if ISOTOPE == 1
c CHANGES took away the multiplied cmymms in the end of the sumoiof
c and  in the end of the sumoiodf...
*
#endif
...
#if ISOTOPE == 1
            sumoiof(i,j) = sumoiof(i,j)*ratm
            sumoidf(i,j) = sumoidf(i,j)*ratm
#endif
