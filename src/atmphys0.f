...
c***  longwave radiation parameterisation, based on a linearisation
c***  of KRCM with respect to reference T and q profiles and other
c***  variables (greenhousegases)
...
c**   amplifcation of freshwater feedback
      lwrqa(:,:,:,:)=lwrqa(:,:,:,:)*AMPWIR
c**   amplifcation of feedback at the equator
...
c***  update moisture profile used in the linearization of the
c***  radiative scheme in the tropics:easy surrogate to a change
c***  mean IR flux in the model without affecting sensitivity
      do ism=1,12
...
c***  read 0-2000 anomalies associated with volcanos
      solarvol(:,:,:)=0.
      if (iscenvol.eq.1) then
c       changement fait par Yoann Sallaz-Damaz pour permettre l'utilisation
c       d'un fichier de forçage volcanique évolutif au cours d'une assimilation
        i=1
        k=1
        do
...
!      write(iuo+99,*) 'Aure - indxo3', indxo3
      write(iuo+99,*) 'O3 value', o3-25.
       call flush(iuo+99)
c*** Update LW reference radiation fluxes using new GHG concentrations
...
c values are used from celest.f (now incorporated in ECBilt)
...
12      format(A12,3i6,5f12.3)
      do i=1,nlat
c zonneconstante is 1370 in sw parameterisatie
...
c-----------------------------------------------------------------------
c *** computes energy fluxes above ocean surface
c *** short wave radiation, long wave radiation, sensible heat flux
...
c *** linearization of RCM with ISCCP D2 1990 clouds
c-----------------------------------------------------------------------
...
c *** aerosol scattering included as a correction on the upward
c *** clear sky fluxes
c *** sk,sr: empirical coefficients Dorland et al, J. Geophys. Res.,102,
c *** 28079-28100, 1997.
c *** smsc: mass scattering coefficient [m2/g]
c *** dso4: change in sulfate aerosol column integrated concentration since
c *** pre-industrial times [g/m2]
...
c        else
c          alb2esn(i,j,nn) = albesn(i,j,nn)
c        endif
          if (alb2esn(i,j,nn).ge.1.) then
c           write(*,*)alb2esn(i,j,nn),i,j,iyear,imonth,iday,zac(nol),
c    &       asup,albesn(i,j,nn),iscensul,tas1(i,j)
            alb2esn(i,j,nn)=1.
          endif
          df=dayfr(i)*solarf(i)
          ireg=irn(i,j,nol)
...
c alternative calculation of upward flux at ground:
c in parameterisation no cross terms are accounted for, which are important for
c upward shortwave radiation at surface and therefore also for net flux
c heswsn(i,j)
...
c (1) downward shortwave radiation at TOA
             if (nn.eq.1)fswdtoa0(i,j)=0.
             fswdtoa2=-ftot(5)*df
             fswdtoa0(i,j)=fswdtoa0(i,j)+(fractn(i,j,nn)*fswdtoa2)
c (2) upward shortwave radiation at TOA
             if (nn.eq.1)fswutoa0(i,j)=0.
             fswutoa2(i,j)=ftot(1)*df
             fswutoa0(i,j)=fswutoa0(i,j)+(fractn(i,j,nn)*fswutoa2(i,j))
c (3) downward shortwave radiation at SURFACE
c            fswdsfc(i,j)=-ftot(8)*df
c (4) upward shortwave radiation at SURFACE
c            fswusfc=(heswsn(i,j)+ftot(8)*df)
...
c *** Chao Chou and Neelin and substantially adapted and extended
c *** for global scale and more
c *** specific ECBILT application by the one and only Michiel Schaeffer
c ***
...
c ***                      direction (64)
c ***
c *** input : dtemp(19,nlat,nlon): temperature anomalies [K] wrt ncep
c ***                              climatology tncep in common lwrscheme
c ***         dqa(nlat,nlon) : anomalies of total prec. water cont. below
c ***                          500 hPa wrt ncep climatology
c ***         tcc(nlat,nlon)  : total cloud cover
c ***         ghg(19) : concentrations of well mixed ghg's (see comphys.h)
c ***
c *** output : ulrad1(nlat,nlon): upward longwave radiation [Wm-2] at
c ***                             toa
c ***          ulrad2(nlat,nlon): net longwave radiation [Wm-2] at
c ***                             (500 hPa)
...
c-----------------------------------------------------------------------
      ...
      do i=1,27
Cdqa    dqreg(i)=qancep(i,ism)**0.3333
...
c-hemispheric dependence of tropospheric ozone forcing
          if (i.le.16) then
           h=1
          else
           h=2
...
Cdqa      dqa=lwrmois(i,j)-dqreg(ireg)
Cdqa      q**1/3-qm**1/3=qm**(1/3-n)*(q**n-qm**n)
          dqa=dqreg(ireg)**(0.3333-EXPIR)*
     *          (lwrmois(i,j)**EXPIR-dqreg(ireg)**EXPIR)
Cdqa      write(iuo+99,'(i3,3F12.5)') i,lwrmois(i,j)-
Cdqa &      dqreg(ireg),dqa,lwrmois(i,j)**0.333-dqreg(ireg)**0.33333
          do l=0,1
            do k=1,7
...
c     ulrad0nm=ec_globalmean(ulrad0n)
c     ulrad1nm=ec_globalmean(ulrad1n)
c     ulrad2nm=ec_globalmean(ulrad2n)
c     ulradsnm=ec_globalmean(ulradsn)
c     dlradsnm=ec_globalmean(dlradsn)

c     write(iuo+37,*)ulrad0nm,ulrad1nm,ulrad2nm,ulradsnm,dlradsnm
...
c *** limit evaporation to available snow or bottom moisture
...
c *** evaporation factor =1 over snow, over wet land maximal 1
c *** care has to be taken in case of a snowcover in order to
c *** conserve heat: the evaporation is constraint to the amount
c *** of snow and moisture available; it can happen that in
c *** one timestep, the remaining snow is sublimated and part
c *** of the latent heat flux is also used to evaporate bottom
c *** moisture: Eflux = (1-sfrac)*Esub + sfrac*Evap
     ...
     &                   (qsurfn(i,j,nld)-q10n(i,j,nld))
                ...
                esubf=alphas*edum
                evapf=alphav*edum
                esnow=min(rowat*adsnow(i,j)*rlatsub/dtime,
     &                  esubf)
     ...
     &                                           emois/(rowat*rlatvap)
                ...
                dc=ec_qsat(pgroundn(i,j,nld),tempsgn(i,j,nld))-q10n(i,j,nld)
...
c ***        uvw10 strength of 10 m wind at gaussian grid for windstress
c-----------------------------------------------------------------------
      implicit none
...
Cdqa      lwrmois(i,j)=rmoisg(i,j)**0.3333
...
c23456789012345678901234567890123456789012345678901234567890123456789012
      subroutine ec_surfmois(nn)
c-----------------------------------------------------------------------
c *** calculates specific humidity at the surface
c-----------------------------------------------------------------------
      implicit none
...
c----------------------------------------------------------------------
c ***calculates the total cloud cover tcc using Axel's new scheme
c----------------------------------------------------------------------

c-----------------------------------------------------------------------
c *** calculates total cloud cover tcc, based on a global climatology
c *** from isccpd2. Michiel Schaeffer may 1998.
c *** diagnostic cloud scheme based on relative humidity and omega
c *** and stability. Jules Beersma
c *** depending on iradcloud, cloud climatology of diagnostic clouds are
c *** used in the calculation of radiative fluxes.
c-----------------------------------------------------------------------
...
c enhance clouds in areas of subsidence inversions
...
c      call system('rm outputdata/atmos/qmtabel.dat')
...
c            hulp(i,j,k)=ec_detqmaxexact(tmount,1,1)
...
c-----------------------------------------------------------------------
      ...
      real*8  qmax,ec_detqmax,dqmdi,dqmdj,dqmdk
...
c        pause 'bad arguments in ec_expint'
        call ec_error(20)
      else if(n.eq.0)then
        ec_expint=exp(-x)/x
      else if(x.eq.0.)then
...
11      continue
c        pause 'continued fraction failed in ec_expint'
        call ec_error(20)
      else
        if(nm1.ne.0)then
          ec_expint=1./nm1
...
13      continue
c        pause 'series failed in ec_expint'
        call ec_error(20)
      endif
      return
      end
...
c-----------------------------------------------------------------------
      ...
      call ec_rggtosp(rmoisg,rmoiss)
      call ec_sptogg(rmoiss,rmoisg,pp)
...
c *** horizontal divergence of moisture
      ...
      call ec_trafluxdiv(hdivmg,rmoiss,rmoisg)
...
c         d1(i,j)=omegg500
          vemoisg(i,j)=0d0
c         d2(i,j)=0d0
c          t500=ec_levtempgp(plevel(2),i,j)
c          qstar=relhum(i,j)*ec_qsat(plevel(2),t500)
c           d3(i,j)=qstar
c           d2(i,j)=t500
...
c      if (iyear.eq.7.and.imonth.eq.1) then
c        write(350) ((real(rmoisg(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(hdivmg(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(vemoisg(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(hdmoisg(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(cormois(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(d1(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(d2(i,j)),j=1,nlon),i=1,nlat)
c        write(350) ((real(d3(i,j)),j=1,nlon),i=1,nlat)
c      endif
      call ec_moisbalance()
c      if (iyear.eq.7.and.imonth.eq.1) then
c        write(350) ((real(rmoisg(i,j)),j=1,nlon),i=1,nlat)
...
c ***         moisture changes due to dynamic and convective rain
              ...
              if (rmoisg(i,j).lt.0d0) then
c                write(99,*) 'moisture less than zero in convec !!!'
c                write(99,*) i,j,rmoisg(i,j),qmax,redrain
                rmoisg(i,j)= 0d0
              endif
...
!               write(*,*) "ici", temp4g(1,1),"=", pot4g,"*",potfac2
  ...
  20      continue
        ...
        enddo
      enddo
c      write(99,*) 'maximum iterations of convection : ',ncmax
...
c23456789012345678901234567890123456789012345678901234567890123456789012
      subroutine ec_moisbalance
c-----------------------------------------------------------------------
c *** fix moisture balance in the atmosphere: due to advection and
c *** spectral truncation, the moisture at a specific point can become
c *** negative. All moisture additions to reset the moisture at zero
c *** have been accumulated in cormois. In this routine these additions
c *** are subtracted from rmoisg. This is done for each latitude
c *** separately to prevent an artificial meridional transport of
c *** moisture. If at a given latitude not enough moisture is available
c *** to accomodate the subtraction, the moisture balance is fixed at
c *** the neighbouring equatorward latitude. The weights pw(nlat,1) are
c *** used to correct for changes in the gridsize with latitude.
c-----------------------------------------------------------------------
      implicit none
...
c *** apply diffusion in stratosphere: timestep smaller than:
c *** 2*diffusiontimescale/462(=21*22) fe 10 days, 1 hour timestep
c-----------------------------------------------------------------------
      implicit none
...
c-----------------------------------------------------------------------
c *** computation of vertical temperature profile
c *** based on reference profiles from NCEP reanalysis data
c *** also used in the LWR parameterisation
c *** assuming temperature anomalies wrt these temperature profiles
c *** vary linearly with log of the pressure
c-----------------------------------------------------------------------
      implicit none
...
c *** 15  850.00  290.298
c *** 16  925.00  294.405
c *** 17 1000.00  299.345
c *** 18 1011.99  300.156
c *** Ps 1013.00  301.265 Ts
...
c *** logarithmic interpolation of temperature anomalies; 200 hPa or
c *** higher the anomalies approach T100 temperature within 3
c *** pressure levels
     ...
     *            + beta(nn)*rlogtl(k)))*0.25
            ...
            enddo
          enddo
c *** from a mean height of 500 hPa from NCEP reanalysis data, the
c *** surface pressure is found using hydrostatic equilibrium
c *** and ideal gas law.
...
c *** Temperature of air near surface is then found by
c *** interpolating for this pressure level the temperature profile calculated
c *** above.
...
c *** for LWR parameterisation temperature anomalies wrt seasonal mean are
c *** required:
          do nn=1,2
            DO k=1,18
...
c-----------------------------------------------------------------------
      ...
      integer i,j
      real*8  gfield(nlat,nlon),sum(nlat),globsum,globfac,ec_globalmean
...
c-----------------------------------------------------------------------
c *** computes temperature distribution in K from geopotential
c *** the mean level is given by tempm
c *** input:  geopg,tempm
c *** output: temp2g,temp4g
c-----------------------------------------------------------------------
      implicit none
...
c *** pv forcing due to advection of temperature by
c *** the divergent wind
...
c-----------------------------------------------------------------------
c *** computes vorticity forcing due to advection of temperature by
c *** divergent wind
c-----------------------------------------------------------------------
      implicit none
...
c --- Input:
c --- ^^^^^^
c ---   - time elapsed since 1950 (>0 after 1950) (annee, units : kyr)
c ---
c --- Output:
c --- ^^^^^^
c ---   - longitude of the perihelion measured from the moving vernal
c ---     point (perh, units : deg)
c ---   - eccentricity (ecc)
c ---   - sine of the obliquity (so)
c ---
c --- Read:
c --- ^^^^^
c ---   File: mbcs2_cor
c ---   ~~~~~
c ---     - amplitude for dvlpt of (e-pi) system (cfr. theoretical part)
c ---       (ae)
c ---     - mean rate for dvlpt of (e-pi) system (cfr. theoretical part)
c ---       (y -> be, units : rad.yr-1)
c ---     - phase for dvlpt of (e-pi) system (cfr. theoretical part)
c ---       (z -> ce, units : rad)
c ---
c ---     - amplitude for dvlpt of obliquity (aob, units : arcsecond)
c ---     - mean rate for dvlpt of obliquity
c ---       (y -> bob, units : rad.yr-1)
c ---     - phase for dvlpt of obliquity
c ---       (z -> cob, units : rad)
c ---
c ---     - amplitude for dvlpt of general precession in longitude
c ---       (aop, units : arcsecond)
c ---     - mean rate for dvlpt of general precession in longitude
c ---       (y -> bop, units : rad.yr-1)
c ---     - phase for dvlpt of general precession in longitude
c ---       (z -> cop, units : rad)
c ---
c --- Write:
c --- ^^^^^^
c ---   File: mout.d (iwr1)
c ---   ~~~~~
c ---     - number of terms for dvlpt of (e-pi) system
c ---       (cfr. theoretical part) (nef)
c ---     - number of terms for dvlpt of obliquity (nob)
c ---     - number of terms for dvlpt of general precession in
c ---       longitude (nop)
c ---     - climatic precession (pre)
c ---     - absolute value of year (or number of page-AB)
c ---       (ipage, units : kyr)
c ---     - year (ikyr, units : kyr)
c ---     - eccentricity (ecc)
c ---     - longitude of the perihelion measured from the moving vernal
c ---       point (perh, units : deg)
c ---     - obliquity (xob, units : deg)
c ---  ?  - caloric equator (xeq, units : deg)
c ---
c ---   File: bcmlog (iwr6)
c ---   ~~~~~
c ---     - year (ikyr, units : kyr)
...
c --- ^^^^^^^^^^^^
c ---   - pi (pi, units : rad)
c ---   - conversion factor deg --> rad (pir, units : rad.deg-1)
c ---   - conversion factor arcsecond --> rad (pirr, units : rad.arcsecond-1)
c ---   - independent term for dvlpt of obliquity (xod, units : deg)
c ---   - independent term for dvlpt of general precession in longitude
c ---     (xop, units : deg)
c ---   - linear term (precessional constant) for dvlpt of general
c ---     precession in longitude (prm, units : arcsecond.yr-1)
c ---   - eccentricity * sine (longitude of perihelion in a fixed
c ---     reference frame) (xes)
c ---   - eccentricity * cosine (longitude of perihelion in a fixed
c ---     reference frame) (xec)
c ---   - argument of the trigonometrical function in the expansions of
c ---     (e-pi) system or general precession in longitude or obliquity
c ---     (arg, units : rad)
c ---   - longitude of perihelion in a fixed reference frame
c ---     (rp, units : rad; drp, units : deg)
c ---   - general precession in longitude (prg, units : arcsecond;
c ---     dprg, units : deg)
c ---
c --- Bibliography:
c --- ^^^^^^^^^^^^^
c ---   - Berger, A., Long-term variations of daily insolation and
c ---     Quaternary climatic changes
c ---     J. Atmos. Sc., 35 (12), 2362-2367, 1978.
c ---
c --- Last changes made (by,when):
c --- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
c ---   DUTRIEUX Alexis and LOUTRE Marie-France, 05/11/1992
c ---
c --- ===================================================================
c
       implicit none
...
c      double precision perh,ecc2,so
c      common /so10/ perh,ecc2,so
      ...
      include 'comrunlabel.h'
c
      real*8    ae(19),be(19),ce(19),
     1          aob(104),bob(104),cob(104),
     2          aop(177),bop(177),cop(177)
      real*8 pi1,pir,pirr,annee,y,z,xod,xop,prm,t,xes,xec,arg,
     1       rp,drp,prg,dprg,pre,xob,xeq
      integer nef,nob,nop,ipage,ikyr,i
c --- -------------------------------------------------------------------
c

c*****change by H. Renssen, 25-02-03
C     annee=(19550.0d0-irunlabelf-iyear)/-1000.0d0
      annee=(+irunlabelf+iyear-1950)/1000.0d0
c*****
c --- 0. Computation of errtst (= needed to resolve error test)
c     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      do i=-16,16,1
         errtst(i)=10.d0**(i)
      enddo
c ----
      ...
      pirr = pir / 3600.0d0
c
c --- 1.earth orbital elements :
c --- ^^^^^^^^^^^^^^^^^^^^^^^^^^
c
C     open(unit=47,status='old',
C    +file='/home/astr/hgs/ecclio_vecode/ecbilt/inputdata/mbcs2_cor')
C     rewind  47
      rewind (iuo+38)
c
c --- Eccentricity:
...
Cdqa      q**1/3-qm**1/3=qm**(1/3-n)*(q**n-qm**n)
     ...
     *          *dumts
              enddo
c             lwrz(7,l)=lwrz(7,l)+
c    *        (lwrts(7,m,ireg,is,l)+lwrqts(7,m,ireg,is,l)*dqa)
c    *        *dumts
              dumts=dumts*(tsurfn(i,j,nn)-tncep(19,ireg,ism))
            enddo
...
*
      ...
      dimension dkhqp(4),psiom(2)
*
calcul de l'inclinaison et du noeud de l'ecliptique puis du vecteur unite
...
*
calcul du vecteur unite normal a l'equateur vrai de la date
*
      xj=sin(psiom(1))*sin(psiom(2))
      yj=-cos(psiom(1))*sin(psiom(2))
      zj=cos(psiom(2))
*
calcul du produit scalaire et de l'obliquite
*
...
*
calcul de e, pi, i, Omega rapportes a l'ecliptique J2000
*
...
*
      ...
      SUBROUTINE ec_irn
c *** =========================================================================
c *** Purpose : update irn variable (surface type) in case of icemask forcing
c *** Original code : C. Van Meerbeek (XXXX)
c *** Modified code : P. Mathiot (01/2012) Allow interannual simulation
c *** =========================================================================
...
c*** 0 : adapt LWR scheme ==>
c          use k=27 Greenland values for icecaps in Northern Hemisphere
cdmr 0--- With use of the T21 ICE-5G icemask ...
...
! READ irn before applied icemask file
      open(111,file='inputdata/lwrref.dat'
     &      ,form='unformatted')
